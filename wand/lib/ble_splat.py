...
class OpenSplat():
    ...
    def _irq_handler(self, event, data):
        ...
        if event == 1:  # _IRQ_CENTRAL_CONNECT
            ...
            if self.target_addr == self.mac_address and not self._connecting:
                self._ble.gap_scan(None)  # pyright: ignore[reportArgumentType]  # None stops scanning; stub mistypes as int
                self._scanning = False
            ...
            if self._start_handle and self._end_handle:
                self._ble.gattc_discover_characteristics(
                    self._conn_handle, self._start_handle, self._end_handle  # pyright: ignore[reportArgumentType]  # conn_handle is int at runtime; stub mistypes as memoryview
                )
            else:
        ...
        if self._rx_char_handle:
            try:
                self._ble.gattc_write(
                    self._conn_handle, self._rx_char_handle + 1, b'\x01\x00'  # pyright: ignore[reportArgumentType]  # conn_handle is int at runtime; stub mistypes as memoryview
                )
                if self._verbose:
    ...
    def _write_command(self, data):
        ...
        try:
            self._ble.gattc_write(self._conn_handle, self._tx_char_handle, data)  # pyright: ignore[reportArgumentType]  # conn_handle is int at runtime; stub mistypes as memoryview
