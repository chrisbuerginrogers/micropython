...
import network
import espnow  # pyright: ignore[reportMissingImports]  # built-in MicroPython module; no stub shipped
import json
import time
...
class ESPNowManager:
    ...
    def shutdown(self):
        ...
        try:
            self.enow.active(False)  # pyright: ignore[reportOptionalMemberAccess]  # non-None while _active
        except Exception:
            pass
        self._active = False
        self._peers.clear()
    ...
    def add_peer(self, mac_str):
        ...
        if mac_str not in self._peers:
            try:
                self.enow.add_peer(mac_bytes)  # pyright: ignore[reportOptionalMemberAccess]  # non-None while _active
            except Exception:
                pass
            self._peers[mac_str] = mac_bytes
    ...
    def remove_peer(self, mac_str):
        if mac_str in self._peers:
            try:
                self.enow.del_peer(self._peers[mac_str])  # pyright: ignore[reportOptionalMemberAccess]  # non-None while _active
            except Exception:
                pass
    ...
    def get_rssi(self, mac_str):
        try:
            ...
            return self.enow.peers_table[mb][0]  # pyright: ignore[reportOptionalMemberAccess]  # AttributeError caught below if enow is None
    ...
    def broadcast(self, data):
        ...
        try:
            self.enow.send(BROADCAST_MAC, msg, False)  # pyright: ignore[reportOptionalMemberAccess]  # non-None while _active
            return True
        except OSError:
            ...
            try:
                ...
                self.enow.send(BROADCAST_MAC, msg, False)  # pyright: ignore[reportOptionalMemberAccess]
                return True
    ...
    def send_to(self, mac_str, data):
        ...
        msg = json.dumps(data) if not isinstance(data, (str, bytes)) else data
        try:
            self.enow.send(mac_bytes, msg)  # pyright: ignore[reportOptionalMemberAccess]  # non-None while _active
            return True
        except Exception as e:
    ...
    def send_raw(self, mac_bytes, raw_bytes):
        if not self._active:
            return False
        try:
            self.enow.send(mac_bytes, raw_bytes)  # pyright: ignore[reportOptionalMemberAccess]  # non-None while _active
            return True
        except Exception as e:
    ...
    def poll(self, timeout_ms=0):
        ...
        try:
            mac, msg = self.enow.irecv(timeout_ms)  # pyright: ignore[reportOptionalMemberAccess]  # non-None while _active
        except Exception:
            return None, None, None
        if msg is None:
            return None, None, None
    ...
    def drain(self):
        ...
        while True:
            try:
                mac, msg = self.enow.irecv(0)  # pyright: ignore[reportOptionalMemberAccess]  # non-None while _active
                if msg is None:
                    break
            except Exception:
                break